...
from .models import Payment, PurchasedCommodity
...
class PayView(TemplateView):
  template_name = "purchase/pay.html"
  ...
  def post(self, request, *args, **kwargs):
    try:
      ...
      params = {
          ...
          "server_url": "https://webstore-eta.vercel.app/pay-callback/",
...
@login_required
@permission_required("purchase.can_form_report", raise_exception=True)
def form_report(request):
  if request.method == "POST":
    ...
    if form.is_valid():
      ...
      return redirect(reverse("purchase:report", args=(start_date, end_date,)))
  else:
    form = ReportForm()

  return render(request, "purchase/formReport.html", {"form": form})
...
def report(request, start_date, end_date):
  ...
  if not purchased_commodities.exists():
    ...
    return redirect(reverse("purchase:form-report"))
  ...
  return render(request, "purchase/report.html", context)
...
def user_payments(request):
  ...
  return render(request, "purchase/userPayments.html", context)
